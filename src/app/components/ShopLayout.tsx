  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-6 border-t border-[#1a1a1a] relative z-[110] bg-[#0a0a0a]/80 backdrop-blur-sm">
          ...
          >
            ...
            CART
            <span className="ml-auto bg-accent text-white text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold leading-none">
      ...
      >
        ...
        {isMounted && cartCount > 0 && (
          <span className="absolute -top-1 -right-1 bg-white text-accent text-xs rounded-full w-5 h-5 flex items-center justify-center font-bold leading-none">
            {cartCount}
          </span>
        )}
      </button>
...
function CartPanel({ isOpen, onClose, cartCount }: CartPanelProps) {
  ...
  // Lock body scroll on mobile when cart is open
  useEffect(() => {
    if (isOpen) {
